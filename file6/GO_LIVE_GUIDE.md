...
- [ ] **3-8. Star3:** 「スター2育成」タブで Star2 を招待・監視できる。**担当する Star2 が条件達成すると「スター2認定」ボタンが出て認定できる**（ハンドシェイク）。
- [ ] **3-9. 施設削除:** エラーなく削除できる（手順1-2の効果）。
- [ ] **3-10. メンター未割当のフォールバック（新規ジム想定）:** super/施設が **メンター無し**で Star1 を直接追加 → その Star1 が条件を100%達成 → アプリの「申請通知」で **施設アカウントの「申請」タブに認定申請が届く** → 施設が承認すると認定される。（Star2 も担当スター3が未割当なら同様に施設へ届く。）
- [ ] **3-11. 施設からの昇格（チェーン構築）:** 施設が上記で認定した Star1 の行にある **「↑昇格」ボタン**で Star2 へ昇格 → 同じアカウントのまま Star2 アプリにログインでき、Star1 を育成・認定できる。（Star2→Star3 も同様。）
...
- [ ] Supabase → **SQL Editor** → `file6/supabase/tighten-rls.sql` を貼り付け → **Run**（1回だけ）。
  ...
  - ⚠️ **2026-08-07 に再更新**: Star3 が担当 Star2 を認定できるよう `tr_update` ポリシーを拡張。**旧版を適用済みでも、この最新版で再度 Run すること**（再実行は安全）。未適用だと Star3 の「スター2認定」がセキュリティ有効化後に失敗します。
- [ ] 確認: エラーなく完了する。
...
- [ ] **Star3:** 「スター2育成」に候補者が**表示される** → Star2 を招待できる → **担当 Star2 の条件達成後「スター2認定」で認定できる**（ハンドシェイク）。
- [ ] **メンター未割当フォールバック:** メンター無しの Star1（および担当スター3の無い Star2）の認定申請が**施設の「申請」タブに届き、施設が承認できる**。
- [ ] **施設からの昇格:** 施設が認定済みトレーナーを**「↑昇格」で Star1→Star2 / Star2→Star3** に昇格でき、昇格後に本人が上位アプリにログインできる。
